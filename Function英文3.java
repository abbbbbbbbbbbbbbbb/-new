...
class Function英文3  implements ActionListener {
	Date now=new Date();
	JButton b1;
    JLabel l1;
	JFrame f = new JFrame("困難英文");
	...
	// 设置文件具体路径以读出写入
	File file = new File("C:\\Users\\Jacky\\Desktop\\ENGLISH3.txt");
		...
		timer.scheduleAtFixedRate(new TimerTask() {
	        int x = 30;
	            ...
	            if (x < 0) {
	                ...
	                if(total==0) new finishbad(); 
					if(total==1) new finishnormal(); 
					if(total==2) new finish(); 
	            }
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(e.getSource() == nextbt)
		{
			...
			if(index==2) {
					timer.cancel();
					if(total==0) {
						new finish_jump_normal(); 
						f.dispose();
						}
					if(total==1) {
						new finishnormal_keep_difficult(); 
						f.dispose();
					}
					if(total==2) {
						new finish(); 
